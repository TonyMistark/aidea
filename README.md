...
- **粘贴保留格式** — 在「直接转换」模式下，从网页/Word/微信编辑器粘贴的富文本自动提取结构性格式（标题层级、加粗、斜体、列表、引用块、链接等），转换为 Markdown
- **任务队列** — 支持后台生成，可发起多个任务，实时查看状态（等待中/生成中/已完成/失败），随时取消或清理
- **主题系统** — 内置 4 种排版主题（微信绿、经典、优雅、简洁），一键切换文章展示样式
...
- **一键复制** — 标题、摘要、封面图提示词和正文均可复制，正文复制带完整样式的 HTML，粘贴到公众号编辑器样式不丢
- **支持自定义 AI 后端** — 兼容 OpenAI API 格式，可配置自定义 endpoint 和模型
- **多 Prompt 管理** — 在设置页创建、编辑、复制、删除多个 AI Prompt，点击切换生效，自由定制写作风格
...
| 微信排版 CSS | md2wechat default 主题 + doocs/md 主题扩展 |
| AI 服务 | OpenAI API 兼容接口 |
| 富文本解析 | NSAttributedString + 自定义 HTML→Markdown 转换器 |
...
│   ├── PromptItem.swift         # Prompt 数据模型
│   ├── TaskManager.swift        # 任务队列管理
│   └── Theme.swift              # 主题系统（4 种内置主题）
├── Services/
│   ├── GenerationService.swift  # AI 生成逻辑
│   └── RichTextParser.swift     # 富文本→Markdown 转换
├── Views/
│   ├── ContentView.swift        # 主界面（任务列表/主题切换）
│   ├── MarkdownWebView.swift    # 微信排版 WebView（动态主题注入）
│   └── SettingsView.swift       # 设置页
└── Assets.xcassets/
docs/                            # App Store 上架文档
docs/superpowers/                # 产品需求文档与主题 CSS 源文件
```
...
在 Settings 页底部可管理多个 **AI Prompt**，点击切换生效，左滑可编辑/复制/删除，修改后对所有新生成文章立即生效。
